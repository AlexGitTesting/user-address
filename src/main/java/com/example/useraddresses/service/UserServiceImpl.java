...
@Service
public class UserServiceImpl implements UserService<User, UserDto, UserModelDto, AddressedUserDto> {
    ...
    public UserDto createUser(UserDto dto) throws IllegalArgumentException, ValidationCustomException {
        ...
        final User newUser = userConverter.convertToDomain(dto);
    ...
    public AddressedUserDto getUser(Long id) {
        ...
        final User user = userRepository.findById(id).orElseThrow(() -> new EntityNotFoundException("User not found"));
    ...
    public AddressedUserDto updateUser(UserDto dto) throws ValidationCustomException, EntityNotFoundException {
        if (dto.getId() == null || dto.getId() < 1) throw new IllegalArgumentException("user.validation.id.not.correct");
        validationService.validate(dto, "UserDto");
        final User user = userRepository.findById(dto.getId()).orElseThrow(() -> new EntityNotFoundException("user not found"));
    ...
    @Override
    @Transactional(readOnly = true)
    public Page<AddressedUserDto> getFilteredUsers(UserQueryFilter filter) throws ValidationCustomException {
        ...
        return allUsers.map(user -> new AddressedUserDto(
                userConverter.convertToDto(user),
                user.getAddresses().stream().map(addressConverter::convertToDto).collect(toSet())));
    }
